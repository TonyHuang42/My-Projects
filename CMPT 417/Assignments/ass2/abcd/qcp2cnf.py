...
def write(clauses, prop_a, prop_b, prop_c, prop_d, order, filename):
    file = open(os.path.join("./output", filename), "w+")
    file.write("p cnf " + str(order*order*order*2) + " " + str(len(clauses)+len(prop_a)+len(prop_b)+len(prop_c)+len(prop_d)) + "\n")
    for line in clauses:
...
if sys.argv[1] != "all":
    ...
    write(qcp2clauses(qcp, order), property_a(order), property_b(order), property_c(order), property_d(order), order, output_file)
