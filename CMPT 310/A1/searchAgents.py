        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextState in self.corners:
                    if nextState not in corners_visited:
...
def cornersHeuristic(state, problem):
    ...
    corners_not_visited = []
    for x in corners:
        if x not in corners_visited:
            corners_not_visited.append(x)
    total_distance = 0
    while corners_not_visited != []:
        distance_list = []
        for x in corners_not_visited:
            distance_list.append(util.manhattanDistance(coordinate, x))
        min_distance = distance_list[0]
        min_index = 0
        index = 1
        while index < len(distance_list):
            if distance_list[index] < min_distance:
                min_distance = distance_list[index]
                min_index = index
            index+=1
        total_distance += min_distance
        coordinate = corners_not_visited.pop(min_index)
